-- ─────────────────────────────────────────────────────────────────────────────
-- Migration 00059: Add government fields to create_transaction_atomic RPC
--
-- Adds five new parameters (all with DEFAULT values, backward-compatible):
--   p_is_government_sale, p_po_number, p_government_agency,
--   p_withholding_rate, p_withholding_amount
--
-- The DROP must target the exact 20-parameter signature from migration 00057.
-- ─────────────────────────────────────────────────────────────────────────────
...
CREATE OR REPLACE FUNCTION create_transaction_atomic(
    ...
    -- Government-specific fields (all optional, backward-compatible)
